...
_REGISTERED_EDGE_CLS = {}
_REGISTERED_EDGE_CFG = {}
...
def register_builder(key: str):
    ...
    """
    return registry.register(_REGISTERED_EDGE_CLS, key)


def build(config: dict, name: str | None = None, **kwargs):
    builder = registry.lookup(_REGISTERED_EDGE_CLS, name)
    return builder(config=config, **kwargs)


def get_config(name: str):
    """Looks up the `Config` according to the `name`."""
    cfg_creater = registry.lookup(_REGISTERED_EDGE_CFG, name)
    return cfg_creater()


def register_config(key: str):
    return registry.register(_REGISTERED_EDGE_CFG, key)
