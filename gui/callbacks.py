...
class ParamCallback:
    def __init__(self, pattern_state:GUIPattern, ui_display, props, key, is_body=False):
        self.pattern_state = pattern_state
        self.ui_pattern_display = ui_display
        self.props = props
        self.key = key
        self.is_body = is_body

        self.NONE = 'Empty'  # TODO Make a global variable or smth
    ...
    def __call__(self, new_val):
        ...
        if new_val == self.NONE:
            new_val = None
        if self.is_body:
            self.props[self.key] = new_val
        else:
            self.props[self.key]['v'] = new_val

        # DEBUG
        print('Param change event!!')
        print(self.key, new_val)
        print(self.props)
        ...
        # DEBUG
        print(f'tmp path server ', self.pattern_state.tmp_path)
...
class GUIState:
        ...
        with ui.scroll_area().classes(f'w-[16vw] h-[{self.h_content - 10}vh]'):   # NOTE: p-0 m-0 gap-0 dont' seem to have effect
            ...
            for param in body:
                ...
                callback = ParamCallback(
                    self.pattern_state, self.ui_pattern_display,
                    body, param,
                    is_body=True
                )
                elem = ui.number(
    ...
    def update_state(self):
        ...
        # TODO Update derivative body values (just in case)

        # DEBUG
        print(self.pattern_state.body_params.params)
