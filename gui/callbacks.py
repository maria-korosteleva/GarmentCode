...
def collapsible(layout, key, title='',
                arrows=(sg.SYMBOL_DOWN, sg.SYMBOL_RIGHT), collapsed=True):
    """
    User Defined Element
    A "collapsable section" element. Like a container element that can be
        collapsed and brought back
    ...
    :param arrows:Tuple[str, str]: The strings to use to show the section is
        (Open, Closed).
    :param collapsed:bool: If True, then the section begins in a collapsed state
    :return:sg.Column: Column including the arrows, title and the layout that
        is pinned

    # NOTE: from https://github.com/PySimpleGUI/PySimpleGUI/blob/master/DemoPrograms/Demo_Column_Collapsible_Sections.py

    """
    return sg.Column([[sg.T((arrows[1] if collapsed else arrows[0]),
                            enable_events=True, k=key+'#BUTTON'),
...
class GUIState:
    ...
    def def_flat_design_layout(self, design_params, pre_key='DESIGN',
            ...
            if 'v' in design_params[param]:
                ...
                if 'select' in p_type:
                    ...
                    in_field = sg.Slider( 
                        ...
                        key=f'{pre_key}#{param}', 
                        # DRAFT enable_events=True # comment to only send
                        # events when slider is released
                    )
                elif 'file' in p_type:
    ...
    def init_body_silhouette(self):
        ...
        self.body_img_id = self.window['CANVAS'].draw_image(
            filename='assets/img/body_30_opacity.png',
            location=self.body_img_margins)
    ...
    def upd_pattern_visual(self):
        ...
        if self.body_img_id is not None:
            self.window['CANVAS'].relocate_figure(
                self.body_img_id, *self.body_img_margins)
    ...
    def input_text_on_enter(self, tag):
        """Modify input text elements to only send events when Enter
        is pressed"""
    ...
    def prettify_sliders(self):
        """ Make slider knowbs flat and small
            A bit of hack accessing lower level library (Tkinter) to reach the
                needed setting
